...
import numpy as np
import joblib  # Ensure you're using joblib for loading models
...
class prediction:
    def regression():
        with st.form('Regression'):
            ...
            with col1:
                item_date = st.date_input(label='Item Date', min_value=date(2020, 1, 1), max_value=date(2021, 12, 31), value=date(2020, 7, 1))
            ...
            with col3:
                delivery_date = st.date_input(label='Delivery Date', min_value=date(2020, 1, 1), max_value=date(2022, 12, 31), value=date(2020, 8, 1))
        ...
        if button:
            with open("C:\\Users\\akash\\PycharmProjects\\copper_modelling\\regression_model.joblib", 'rb') as f:
                model = joblib.load(f)  # Use joblib to load the model
    ...
    def classification():
        ...
        if button:
            with open("C:\\Users\\akash\\PycharmProjects\\copper_modelling\\classification_models.joblib", 'rb') as f:
                model = joblib.load(f)  # Use joblib to load the model
